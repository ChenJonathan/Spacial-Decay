...
public class TutorialWave : Wave
{
    ...
    private IEnumerator Run()
    {
        ...
        yield return StartCoroutine(transmission.ShowContent("Plot warp jumps using your HUD by selecting a target destination. Keep the primary click engaged; the computer will calculate and render the warp jump for you.", 0.05f));
        yield return StartCoroutine(transmission.ShowContent("You may cancel a jump through an alternate click input, but as the engines have already been overloaded, the ship will still fire the engines without consuming extra charge.This will result in a sub-warp velocity.", 0.05f));
        ...
        yield return StartCoroutine(transmission.ShowContent("As a result, you may freely pass through both steel and plasma, ignoring collisions with enemy ships or enemy projectiles.", 0.05f));
        ...
        yield return StartCoroutine(transmission.ShowContent("As you accumulate experience as a pilot, you will be sent deeper into hostile space near the front lines.", 0.05f));
